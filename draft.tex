...
As of \textbf{ADD DATE} with the completion of the Human Genome Project this has been accomplished\textbf{CITATION NEEDED}.
The human genome consists of approximately 3.3 billion basepairs by simply cross referencing to a healthy genome, single point mutations can effortlessly be identified, especially with modern high-throughput analysis tools\cite{rawal}.
Cataloguing such mutations is useful but in itself provides little insight, as chances are the mutation occured a noncoding part.
...
Somatic mutations occur in the order of magnitute of $ 2*\frac{number of cell divisions}{10^8 base pairs}$, which with 30 trillion cells equates roughly 500 trillion per year\cite{somaticmutation}.
...
As discussed in the following chapter, subpar treatment grounded in genome analysis results is no better than discrimination on the basis of sex, race, age or religion.
